...
public class MinCoinCount2 {
    int getMinCoinCountOfValue(int total, int[] values, int valueIndex) {
        ...
        for (int count = maxCount; count >= 0; count--) {
            ...
            if (restCount == Integer.MAX_VALUE) { // 如果当前面值已经为0，返回-1表示尝试失败
                if (count == 0) {
                    break;
                } // 否则尝试把当前面值-1 continue; }minResult = Math.min(minResult, count + restCount); }return minResult; }
            }
    ...
    public static void main(String[] args) {
        int res = new MinCoinCount2().getMinCoinCountOfValue();
